/*
  # Create stays and configuration tables
  ...
  1. New Tables
    ...
    - `app_config`
      ...
      - `year` (integer)
      - `month` (integer)
      - `default_daily_tax` (decimal)
      - `created_at` (timestamp)
      - `updated_at` (timestamp)

  2. Security
    - Enable RLS on both tables
    - Add policies for public access (since this is a single-user B&B app)
*/
...
CREATE TABLE IF NOT EXISTS stays (
  ...
  daily_tax decimal(10,2) NOT NULL CHECK (daily_tax >= 0),
...
CREATE TABLE IF NOT EXISTS app_config (
  ...
  default_daily_tax decimal(10,2) NOT NULL DEFAULT 2.00 CHECK (default_daily_tax >= 0),
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
);
...
-- Create policies for public access (single-user B&B app)
